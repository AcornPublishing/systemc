...
//
// This is a refined version of the source_sink example.
// The primitive FIFO channel (sc_fifo) was replaced by
// a FIFO module with a pin-level interface. In order
// to connect the SOURCE and SINK modules we had to
// introduce adapters (FIFO_WRITE_HS and FIFO_READ_HS,
// respectively). See section 9.3 for details.
//
...
// This adapter converts the FIFO write interface into a bi-
// directional handshake protocol. It is a hierarchical channel
// that implements the sc_fifo_out_if interface. FIFO_WRITE_HS
...
public:
    ...
    // NB: data_read_event() was introduced with SystemC 2.0.1
...
int sc_main(int argc, char* argv[])
{
    ...
    FIFO_WRITE_HS<int> write_adapter("write_adapter");
    ...
    // interconnect
    source.output(write_adapter);
    sink.input(read_adapter);
    write_adapter.clock(clock);
    write_adapter.data(source_data);
    write_adapter.ready(source_ready);
    write_adapter.valid(source_valid);
