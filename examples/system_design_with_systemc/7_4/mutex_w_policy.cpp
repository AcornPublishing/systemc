...
class mutex_w_policy : public sc_mutex 
{ 
public: 
  enum policy_type { FIFO, RANDOM };

  explicit mutex_w_policy(policy_type policy) : policy_(policy) {}
...
public:
  ...
  top(sc_module_name name) : mutex_(mutex_w_policy::FIFO)
  {
    SC_THREAD(t1);
    SC_THREAD(t2);
    SC_THREAD(t3);
