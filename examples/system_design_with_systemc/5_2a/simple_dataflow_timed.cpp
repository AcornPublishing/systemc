...
//
// Timed functional version of the simple_dataflow example.
// Processing delays have beeen addded to constant generator
// and adder. The DF_Printer module has been changed in order
// to also print the (simulated) time.
//
...
template <class T> SC_MODULE(DF_AdderTimed) { 
    ...
    SC_CTOR(DF_AdderTimed) { SC_THREAD(process); } 
}; 
